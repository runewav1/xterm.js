...
export class GpuRenderer extends Disposable implements IRenderer {
  ...
  private readonly _backend: IGpuBackend;
  private _rectangleRenderer: MutableDisposable<IRectangleRenderer> = this._register(new MutableDisposable());
  private _glyphRenderer: MutableDisposable<IGlyphRenderer> = this._register(new MutableDisposable());
  ...
  constructor(
    ...
    backendFactory: (canvas: HTMLCanvasElement) => IGpuBackend
  ) {
    super();

    // IMPORTANT: Canvas initialization and fetching of the context must be first in order to
    // prevent possible listeners leaking and continuing to operate after the GpuRenderer has been
    // discarded.
    this._canvas = this._coreBrowserService.mainDocument.createElement('canvas');
    this._backend = this._register(backendFactory(this._canvas));

    this._register(toDisposable(() => {
      this._canvas.remove();
      releaseTextureAtlas(this);
    }));
    try {
      ...
      this._renderLayers = [
        this._register(new LinkRenderLayer(this._core.screenElement!, 2, this._terminal, this._core.linkifier!, this._coreBrowserService, _optionsService, this._themeService))
      ];
      ...
      this._register(EventUtils.forward(this._backend.onContextLoss, this._onContextLoss));
      if (this._backend.onContextRestored) {
        this._register(this._backend.onContextRestored(() => {
          releaseTextureAtlas(this);
          this._initializeBackendState();
          this._requestRedrawViewport();
        }));
      }
  ...
  /**
   * Initializes members dependent on backend context state.
   */
  private _initializeBackendState(): void {
    const { rectangleRenderer, glyphRenderer } = this._backend.createRenderers(this._terminal, this.dimensions, this._optionsService, this._themeService, this._logService);
    this._rectangleRenderer.value = rectangleRenderer;
    this._glyphRenderer.value = glyphRenderer;
  ...
  private _refreshCharAtlas(): void {
    ...
    const atlas = acquireTextureAtlas(
      ...
      this._backend.maxTextureSize,
      this._backend.maxAtlasPages,
      this._customGlyphs
    );
